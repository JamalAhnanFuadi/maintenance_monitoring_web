package id.tsi.mmw.controller;

import id.tsi.mmw.model.Staff;
import org.jdbi.v3.core.Handle;
import org.jdbi.v3.core.statement.Query;
import org.jdbi.v3.core.statement.Update;

import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

public class MasterUserGroupController extends BaseController {
    public MasterUserGroupController() {
        log = getLogger(this.getClass());
    }

    public List<Staff> getUserList() {
        final String methodName = "getUserList";
        start(methodName);
        List<Staff> result = new ArrayList<>();
        String sql = "Select uid, group_key, description ,create_dt, modify_dt from master_user_group";
        try (Handle handle = getHandle(); Query q = handle.createQuery(sql)) {
            // Execute the query and get the result as a Boolean
            result = q.mapToBean(Staff.class).list();
        } catch (SQLException e) {
            // Log any SQL exception that occurs during the authentication process
            log.error(methodName, e);
        }

        completed(methodName);
        return result;

    }

    public boolean create(Staff staff) {
        final String methodName = "create";
        start(methodName);
        boolean result = false;
        final String sql = "INSERT INTO [master_user_group] (uid, group_key, description ,create_dt, modify_dt)values (:uid, :group_key, :description ,:create_dt, :modify_dt)";

        try (Handle h = getHandle(); Update u = h.createUpdate(sql)) {
            u.bindBean(staff);
            result = executeUpdate(u);
        } catch (Exception ex) {
            log.error(methodName, ex);
        }
        completed(methodName);
        return result;

    }

    public boolean update(Staff staff) {
        final String methodName = "update";
        start(methodName);
        boolean result = false;
        final String sql =
                "UPDATE [master_user_group] SET uid =:uid, group_key =: group_key, description =: description ,create_dt =: create_dt, modify_dt =: modify_dt ";
        try (Handle h = getHandle(); Update u = h.createUpdate(sql)) {
            u.bindBean(staff);
            result = executeUpdate(u);
        } catch (Exception ex) {
            log.error(methodName, ex);
        }
        completed(methodName);
        return result;
    }

    public boolean delete(String uid) {
        final String methodName = "delete";
        start(methodName);
        boolean result = false;
        final String sql = "DELETE FROM [master_user_group] WHERE uid = :uid";
        try (Handle h = getHandle(); Update u = h.createUpdate(sql)) {
            u.bind("uid", uid);
            result = executeUpdate(u);
        } catch (Exception ex) {
            log.error(methodName, ex);
        }
        completed(methodName);
        return result;
    }

    public Staff getUserByUid(String uid) {
        final String methodName = "getUserByUid";
        start(methodName);

        Staff staff = new Staff();
        final String sql = "SELECT uid, group_key, description ,create_dt, modify_dt from master_user_group" +
                " FROM master_user_group WHERE uid = :uid";
        try (Handle h = getHandle(); Query q = h.createQuery(sql)) {
            q.bind("uid", uid);
            staff = q.mapToBean(Staff.class).one();
        } catch (Exception ex) {
            log.error(methodName, ex);
        }
        completed(methodName);
        return staff;
    }
}
